...
export default function LogDetailsModal({ log }: LogDetailsModalProps) {
	return (
		<DialogContent className="max-h-[80vh] max-w-2xl overflow-y-auto border-border bg-accent dark:bg-card">
			<DialogHeader>
			...
			<div className="space-y-6 ">
				<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
					<div className="space-y-2">
						...
						<div className="rounded bg-muted p-2 font-mono text-foreground text-sm">
							{log.id}
						</div>
					</div>
					<div className="space-y-2">
						<div className="font-medium text-foreground text-sm">Group</div>
						<div className="rounded bg-muted p-2 text-foreground text-sm">
							{log.group}
						</div>
					</div>
				</div>
